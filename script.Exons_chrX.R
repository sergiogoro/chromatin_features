...
#nrow(data)
data <- subset(data, chromosome == "X")                     # Chromosome X
Erep <- (data[["num_FBtrs"]] / data[["FBtrs_per_gene"]])  # Exones donde anotaciones 5.49 y 5.50 son distintos!!    #Solo para data exon.shortintrons.n
...
### EXON ###
	...
	### Transcripts (mRNAs per exon) ###
        ...
        png( "EXONS-chrX-FEAT1Transcripts_omega4f-numFBtrs.png", width = 1920, height = 1080 )
        ...
        png( "EXONS-chrX-FEAT1Transcripts_omegains-numFBtrs.png", width=1920, height=1080 )
        ...
        cat( cor.test(omega_4f, data$num_FBtrs, method="spearman")$estimate, labels="Spearman  (omega_4f ~ data$num_FBtrs)"OUT_EXONS-chrX", fill=T, append=T)
        cat( cor.test(omega_4f, data$num_FBtrs, method="spearman")$p.value, labels="Spearman  (omega_4f ~ data$num_FBtrs)"OUT_EXONS-chrX", fill=T, append=T  )
	...
	### Exon Representativity ### 
        ...
        png( "EXONS-chrX-FEAT2Erep_Histogram.png", width = 1920, height = 1080 )
        ...
        cat( cor.test(omega_4f, Erep, method="spearman")$estimate, file = "OUT_EXONS-chrX",fill=T, labels="Spearman Rho (omega_4f ~ Erep)", append=T)
        cat( cor.test(omega_4f, Erep, method="spearman")$p.value, file = "OUT_EXONS-chrX", fill=T, labels="Spearman p-value (omega_4f ~ Erep)", append=T)
	...
	### Order ###
        ...
        png( "EXONS-chrX-FEAT3Order_m-orderIntervals.png", width = 1920, height = 1080 )
        boxplot(m~order_intervals,outline=F,xlab="Exon Order",ylab="m") 
        dev.off()

        cat( cor.test(data$order, omega_4f, method="spearman")$estimate, file = "OUT_EXONS-chrX",fill=T, labels="Spearman Rho (data$order ~ omega_4f)", append=T)
        cat( cor.test(data$order, omega_4f, method="spearman")$p.value, file = "OUT_EXONS-chrX", fill=T, labels="Spearman p-value (data$order ~ omega_4f)", append=T)
	...
	### Exon Length ###
        ...
        png( "EXONS-chrX-FEAT4Elength_omegains-m3.png", width = 1920, height = 1080 )
	    boxplot( omega_ins~m3, outline=F, xlab="Exon Length", ylab="Ka/Kins")
	    dev.off()

        cat( cor.test(m, omega_4f, method="spearman")$estimate, file = "OUT_EXONS-chrX",fill=T, labels="Spearman Rho (m ~ omega_4f)", append=T)
        cat( cor.test(m, omega_4f, method="spearman")$p.value, file = "OUT_EXONS-chrX", fill=T, labels="Spearman p-value (m ~ omega_4f)", append=T)
